...
import { CreditCard, Eye, EyeOff, Loader2, CheckCircle2, ArrowRight, Mail } from "lucide-react";
...
const Register = () => {
  ...
  const handleGoogleRegister = async () => {
    await lovable.auth.signInWithOAuth("google", {
      redirect_uri: `${window.location.origin}/onboarding-business`,
    });
  };
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        ...
        emailRedirectTo: `${import.meta.env.VITE_APP_URL || window.location.origin}/onboarding-business`,
      },
    });
    ...
    if (data.session) {
      toast.success("Compte créé ! Configurez maintenant votre commerce.");
